...
#'
#' @export
brix <- function(start_value, ll, formula = NULL,
  response = NULL, design = NULL, data, d = 0, ...) {
  ...
  o <- stats::optim(par = start_value,
    fn = ll$fun,
    control = list(fnscale = -1, maxit = 1e+06),
    hessian = TRUE,
    z = z,
    x = x,
    d = d)
  ...
  names(o$coef) <-
    names(o$par) <-
    c(colnames(z), ll$name) #names of beta coefficients
  ...
  null <- stats::optim(par = rep(0.1, 1 + extra),
    fn = ll$fun,
    control = list(fnscale = -1, maxit = 1e+06),
    z = z_null,
    x = x,
  ...
  id_family <- eval(parse(text = paste0(ll$info$family,
                                        "(link = \"identity\")")))
  id_family_log_likelihood <- do.call(what = log_likelihood,
                                      args = list(fam = id_family))

  if (extra == 0) {
    null_beta <- rep(1, length(x))
  } else {
    null_beta <- c(rep(1, length(x)),
                   start_value[ - (1:(length(start_value)) - start_value)])
  }

  o$null.deviance <-
    2 * (do.call(what = id_family_log_likelihood$fun,
                 args = list(x = x, beta = null_beta, z = diag(x)))
       - do.call(what = ll$fun,
                 args = list(x = x, beta = null$par, z = z_null)))
  ...
  o$deviance <-
    2 * (do.call(what = id_family_log_likelihood$fun,
                 args = list(x = x, beta = null_beta, z = diag(x)))
       - do.call(what = ll$fun,
                 args = list(x = x, beta = o$par, z = o$z)))
...
print.summary.brix <- function(x, ...) {
  ...
  cat("\nResidual Deviance:",
      x$deviance, " on", x$df.residual, " degrees of freedom\n")
